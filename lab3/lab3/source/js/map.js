...
function map(data, world_map_json){
  ...
  this.filterTime = function (value) {
      ...
      d3.selectAll(".point").data(data)
              ...
              .style("opacity", 0.3)
              .style("visibility", function (d) {
                  //push d to filterdData only if mag is > curr_mag and tmpT is between timeExt
                  var tmpT = format(d.time);
			  if ( d.mag > curr_mag && timeExt[0] < tmpT  && tmpT < timeExt[1])					
                  {
                      //filterdData here
  ...
  this.cluster = function () {
	  ...
	  for(d in filterdData){
		var temp =[];
		for(j in filterdData[0]){
			//console.log(j);
			if(j=="lat"){
				temp[0] = filterdData[d].lat;
			}
			if(j== "lon"){
				temp[1] = filterdData[d].lon;
			}
		}
		test.push({
			lat: temp[0],
